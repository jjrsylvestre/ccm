...
[
	...
	\node[above left] (0,1) {$1$};
	\draw[dashed] (-1,-1) -- (0,-1) node[right] {$-1$};
	\foreach \y in {-1,1} {
		\draw (-0.1,\y) -- (0.1,\y);
	}

	\draw[red,thick] (\xmin,1) -- (\xmax,);
	\foreach \x in {-3,1,5,9} {
		\node[red,closedpt] at (\x,1) (pt) {};
		\draw[red,dashed] (pt) -- (\x,0);
	}
	% \node[red,closedpt] at (1,1) {};
	% \node[red,closedpt] at (5,1) {};
	% \node[red,closedpt] at (9,1) {};
