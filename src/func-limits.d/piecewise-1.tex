% *********************************************************************
% © 2024 Jeremy Sylvestre
%
% Permission is granted to copy, distribute and/or modify this document
% under the terms of the GNU Free Documentation License, Version 1.3 or
% any later version published by the Free Software Foundation; with no
% Invariant Sections, no Front-Cover Texts, and no Back-Cover Texts. A
% copy of the license is included in the appendix entitled “GNU Free
% Documentation License” that appears in the output document of this
% PreTeXt source code. All trademarks™ are the registered® marks of
% their respective owners.
%
% *********************************************************************

\newcommand*{\xmin}{-1.5}
\newcommand*{\xmax}{3}
\newcommand*{\ymin}{-0.5}
\newcommand*{\ymax}{2.5}

\begin{tikzpicture}
[
	openpt/.style={circle,draw,inner sep=0pt,minimum size=4pt}
]

	% grid
	\foreach \x in {-1.5,-1,...,3} {
		\draw[very thin,gray] (\x,\ymin) -- (\x,\ymax);
	}
	\foreach \y in {-0.5,0,0.5,...,2.5} {
		\draw[very thin,gray] (\xmin,\y) -- (\xmax,\y);
	}

	% label grid
	\foreach \x in {-1,1} {
		\node[below] at (\x,0) {$\x$};
	}
	\foreach \y in {1,2} {
		\node[left] at (0,\y) {$\y$};
	}

	% axes
	\draw[->,thick] (\xmin,0) -- (\xmax,0) node[below right] {$t$};
	\draw[->,thick] (0,\ymin) -- (0,\ymax) node[above right] {$f(t)$};

	% points
	\node[blue,openpt] at (1,2) (lh) {};
	\node[blue,openpt] at (1,0) (rh) {};

	% parabola graph
	\draw[thick, blue, domain=-1.12:0.985,smooth] plot (\x,{2 * \x * \x});

	% linear graph
	\draw[thick, blue] (rh.north east) -- (3,2);

\end{tikzpicture}
